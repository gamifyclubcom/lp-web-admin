...
const PoolWithdrawal: React.FC<Props> = ({ pool, setLoading }) => {
  ...
  const handleWithdraw = async () => {
    try {
      ...
      const rawTx = await action.withdrawByAdmin(
        walletKey,
        walletKey,
        contractPublicKey
      );
  ...
  const fetchBalance = async () => {
    try {
      ...
      setTokenPerBalance(
        await getBalance(connection, action, new PublicKey(pool!.token_x))
      );
      setTokenSalesBalance(
        await getBalance(connection, action, new PublicKey(pool!.token_y))
      );
    ...
    <Card>
      ...
      <CardContent style={{ paddingRight: '8%', paddingLeft: '8%' }}>
        <BalancesTable
          data={[
            {
              token_name: pool?.token_to || '',
              amount: tokenPerBalance,
              fee_percentage: `${feeSetting}%`,
              fee: round(
                new Decimal(soldAmount).mul(feeSetting).div(100).toNumber(),
                9
              ),
            },
            {
              token_name: pool?.token_symbol || '',
              amount: tokenSalesBalance,
              fee_percentage: '0%',
              fee: 0,
            },
          ]}
...
const BalancesTable: React.FC<{ data: RowData[]; handleWithdraw: () => void }> =
  ({ data, handleWithdraw }) => {
